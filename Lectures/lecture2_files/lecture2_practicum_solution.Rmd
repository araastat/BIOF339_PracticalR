...
```{r, echo=FALSE}
knitr::opts_chunk$set(eval = TRUE, echo = TRUE) # Suppress all code and solutions
```
...
4. Try to also read the file directly from the web with the `read_excel` package from the web URL __(My bad: this can't be done. You need to download the file first. However, this can be done for a CSV file using read.csv)
...
or
```{r}
dat_malignant <- subset(dat, Status=='malignant')
dat_benign <- subset(dat, Status == 'benign')
```
