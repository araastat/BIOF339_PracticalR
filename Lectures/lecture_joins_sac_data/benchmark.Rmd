...
expression data set as we did in [Lecture 6](../lecture_joins_sac.html). However, now, we will use the full
...
```{r}
unzip('../lecture_joins_sac_data/fullExpression.zip', list=T) # just lists files
```
...
```{r}
datafile <- unz('../lecture_joins_sac_data/fullExpression.zip',
...
if(!file.exists('BreastCancer_Expression_full.csv')){ # Check if the file already exists
  unzip('../lecture_joins_sac_data/fullExpression.zip',
...
benchmark_reading <- 
  benchmark('read.csv' = read.csv('BreastCancer_Expression_full.csv', stringsAsFactors=F),
          ...
          'read.csv (zipped)' = read.csv(unz('../lecture_joins_sac_data/fullExpression.zip',
                                             'BreastCancer_Expression_full.csv'),
                                         stringsAsFactors=F),
          'read_csv (zipped)' = read_csv(unz('../lecture_joins_sac_data/fullExpression.zip',
                                             'BreastCancer_Expression_full.csv')),
